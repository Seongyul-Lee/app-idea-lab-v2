...
- `ideas/NNN-아이디어명-review.md` — PRD 문서 정합성 판별 리뷰
...
사용자가 **채택**, **3단계 회귀**, **2단계 회귀**, **탈락**, **보류** 중 하나를 선택했다면 아래 해당 섹션을 수행한다.
...
3. **Frontmatter 업데이트** (아래 "Frontmatter 업데이트" 섹션 참조)
...
4. **Frontmatter 업데이트** (아래 "Frontmatter 업데이트" 섹션 참조)

### 3단계 회귀
1. 관련 파일(`NNN-아이디어명.md`, `-prd.md`, `-review.md`)이 `ideas/adopted/`에 있다면 `ideas/`로 **이동**
2. 아이디어 문서 판정란에 `3단계 회귀` 기록 및 회귀 사유 요약
3. 리뷰 보고서(`-review.md`)는 유지 — 수정 지침 참조용
4. `/stage-3 NNN-아이디어명` 실행 안내

### 2단계 회귀
1. 관련 파일(`NNN-아이디어명.md`, `-prd.md`, `-review.md`)이 `ideas/adopted/`에 있다면 `ideas/`로 **이동**
2. 아이디어 문서 판정란에 `2단계 회귀` 기록 및 회귀 사유 요약
3. 기존 PRD(`-prd.md`)와 리뷰 보고서(`-review.md`)를 **삭제** — 2단계부터 재평가하므로 새로 작성
4. `/stage-2 NNN-아이디어명` 실행 안내

### 보류
1. 관련 파일(`NNN-아이디어명.md`, `-prd.md`, `-review.md`)이 `ideas/adopted/`에 있다면 `ideas/`로 **이동**
2. `ideas/` 루트에 유지, 판정란에 `보류` 기록 및 미달 항목·추가 조사 방향 명시
3. 재개 시 `/stage-2 NNN-아이디어명`부터 재시작

---

## Frontmatter 업데이트

리뷰 완료 및 판정 결과 처리 후 아래 2개 파일의 frontmatter를 채운다.

1. **리뷰 보고서** (`NNN-아이디어명-review.md`):
   - `id`, `name`: 해당 값
   - `type`: `"review"`
   - `verdict`: 자동 판정 결과 (`"pass"` | `"regression-3"` | `"regression-2"` | `"reject"`)
   - `final_decision`: 사용자 최종 선택 (`"adopted"` | `"rejected"`)
   - `critical_count`, `major_count`, `minor_count`: 집계
   - `date`: 오늘 날짜

2. **아이디어 카드** (`NNN-아이디어명.md`):
   - `current_stage`: `"stage-4"`
   - `updated_at`: 오늘 날짜
   - `review.verdict`: 자동 판정 결과
   - `review.final_decision`: 사용자 최종 선택
   - `review.date`: 오늘 날짜
   - `review.critical_count`, `review.major_count`, `review.minor_count`: 집계
   - `review.areas`: 4개 영역별 통과/미달 (`business`, `risk`, `technical`, `documentation` — 각각 `"pass"` | `"fail"`)
   - 채택 시: `status`를 `"adopted"`로 변경
   - 탈락 시: `status`를 `"rejected"`로, `rejection.stage`를 `"stage-4"`, `rejection.reason`에 사유 기록
