# 살았니 (SafePin) — PRD v1.2

> 작성일: 2026-01-31
> 최종 수정: 2026-01-31 (v2 리뷰 Minor 7건 반영)
> 상태: Review Ready
> 대상: 003-살았니
...
- **Demumu 바이럴**: 2026년 1월 중국 유료 1위, CNN·BBC·Reuters 보도. 한국 내 네이버 뉴스 보도 0건 → 한국어 서비스 공백 (시장분석 §2)
...
> v1.2: v2 리뷰 Minor 7건(A-M1, B-M1, C-M1, C-M2, C-M3, D-M1, D-M2) 반영 완료
...
| API | `GET /checkins?month=YYYY-MM` (월별 체크인), `GET /alert-logs?month=YYYY-MM` |
...
| API | `GET /contacts`, `POST /contacts`, `PATCH /contacts/:id`, `DELETE /contacts/:id` |
...
| API | `POST /settings`, `POST /contacts` |
...
| API | `POST /checkins` (취소=체크인 처리), `POST /alerts/cancel` |
...
| API | IAP (react-native-iap), `POST /subscriptions/verify` |
...
1. **선점 우위**: 한국 시장 최초 청장년 안부확인 앱으로 브랜드 인지도 확보. Demumu 바이럴 타이밍에 편승
